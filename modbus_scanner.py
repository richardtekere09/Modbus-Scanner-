    ...
    for description, (start_addr, count) in REGISTER_MAP.items():
        try:
            result = client.read_holding_registers(
                address=start_addr, count=count, unit=slave_id
            )
            ...
            if not result.isError() and hasattr(result, "registers"):
                ...
                if non_zero_regs:
                    ...
                    try:
                        ...
                        text = "".join(
                            chr(b) if 32 <= b < 127 else "." for b in bytes_data
                        )
...
print(
    f"[INFO] Found {len(available_ports)} COM port(s): {', '.join(available_ports)}\n",
    flush=True,
)
